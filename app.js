...
const state = {
  ...
  map: null,
  notifications: JSON.parse(localStorage.getItem("trekSathiNotifications") || "null") || [
    {id:1,title:"Friend request received",text:"Riya Lama sent you a friend request.",time:"Just now",unread:true,type:"friend"},
...
function estimatedUSD(npr){
  ...
  const lower = Math.floor(usd / 50) * 50;
  const adjustedLower = lower > 0 ? lower : 25;
  const upper = adjustedLower + 50;

  return usd < 25 ? `${adjustedLower}-${upper}` : `${adjustedLower.toLocaleString()}-${upper.toLocaleString()}`;
}
...
function trailCard(t){
  ...
  return `<article class="trail-card" onclick="openTrail(${t.id})">
    <div class="trail-image" style="background-image:url('${t.image}')"><div class="trail-top">${badge(t.difficulty,t.difficulty.toLowerCase())}<button class="save-btn ${saved?"saved":""}" onclick="event.stopPropagation();toggleSave(${t.id})">${saved?'<i class="fa-solid fa-heart"></i>':'<i class="fa-regular fa-heart"></i>'}</button></div></div>
    <div class="trail-body">
      <div class="eyebrow">${esc(t.province)} · ${esc(t.district)}</div>
      <h3>${esc(t.name)}</h3>
      <p class="muted clamp">${esc(t.desc)}</p>
      <div class="trail-meta"><span><i class="fa-regular fa-clock"></i> ${t.days} day${t.days>1?"s":""}</span><span><i class="fa-solid fa-arrow-up"></i> ${t.altitude}</span><span><i class="fa-solid fa-star"></i> ${t.rating}</span><span class="trail-cost">NPR ${t.cost.toLocaleString()} · USD ${estimatedUSD(t.cost)}</span></div>
...
function userCard(u){
  ...
  return `<article class="person-card">
    ...
    <div class="person-stats">${badge(u.experience,"soft")} <span><i class="fa-solid fa-language"></i> ${u.languages.slice(0,2).map(esc).join(", ")}</span></div>
...
function addNotification(title,text,type="general"){
...
function updateNotificationDot(){
  ...
  if(dot) dot.classList.toggle("hidden",!state.notifications.some(n=>n.unread));
}

function closeNotificationPanel(){
  document.getElementById("notification-panel")?.remove();
  document.getElementById("notification-panel-backdrop")?.remove();
}
...
function renderNotificationPanel(){
  ...
  backdrop.id="notification-panel-backdrop";
  ...
  panel.innerHTML=`
    <div class="notification-head">
      ...
      <button class="notification-close" aria-label="Close notifications">×</button>
    </div>
    <div class="notification-list">
      ${state.notifications.length ? state.notifications.map(n=>`
        <div class="notification-item ${n.unread?"unread":""}">
          <div class="notification-icon"><i class="fa-solid ${n.type==="friend"?"fa-user-plus":n.type==="group"?"fa-users":"fa-circle-info"}"></i></div>
          <div class="notification-copy">
            <b>${esc(n.title)}</b>
            <p>${esc(n.text)}</p>
            <small>${esc(n.time)}</small>
          </div>
        </div>`).join("") : `<div class="notification-empty"><b>No notifications</b><p>You're all caught up.</p></div>`}
    </div>
    ${state.notifications.some(n=>n.unread)?`<button class="btn secondary notification-read" onclick="markNotificationsRead()">Mark all as read</button>`:""}
  `;

  document.body.append(backdrop,panel);
  panel.querySelector(".notification-close").addEventListener("click",closeNotificationPanel);

  // Opening the panel counts the notifications as seen.
...
function renderExplore(){
  ...
  <section class="explore-layout"><aside class="filter-panel">
    ...
    <div class="filter-note"><i class="fa-solid fa-lightbulb"></i> <b>Tip:</b> Save trails to build your personal shortlist.</div>
...
function filterTrails(){
  ...
  document.getElementById("explore-results").innerHTML=arr.length?arr.map(trailCard).join(""):`<div class="empty full-span"><span class="empty-icon"><i class="fa-solid fa-mountain"></i></span><h3>No trails found</h3><p>Try changing your filters.</p></div>`;
}
...
function renderWeatherGrid(targetId, days){
  ...
  el.innerHTML = days.map(d=>{
    ...
    return `<div class="weather-day"><small>${dayName}</small><div class="weather-emoji"><i class="fa-solid ${weatherCodeToIcon(d.code)}"></i></div><div class="weather-temps"><b>${Math.round(d.max)}°C</b><small>${Math.round(d.min)}°C</small></div><small class="muted">${d.precip?d.precip+' mm':''}</small></div>`;
  }).join('');
}
...
function groupCard(g){
  ...
  return `<article class="group-card">
    <div class="group-avatar">${esc(g.avatar)}</div>
    <div class="group-card-top">
      <div><span class="eyebrow">${esc(g.trail)}</span><h3>${esc(g.name)}</h3></div>
      ${badge(g.level,"soft")}
    </div>
    <p>${esc(g.desc)}</p>
    <div class="group-meta">
      <span><i class="fa-regular fa-calendar"></i> ${esc(g.date)}</span>
      <span><i class="fa-solid fa-users"></i> ${g.members}/${g.max}</span>
    </div>
    <div class="group-owner">${avatar({initials:initials(g.owner),name:g.owner},"sm")} <span>Organized by ${esc(g.owner)}</span></div>
    <button class="btn ${joined?"secondary":"primary"} full" onclick="joinGroup(${g.id})">${joined?"Joined":"Join group"}</button>
  </article>`;
}
function joinGroup(id){ state.user.joinedGroups=state.user.joinedGroups||[]; if(!state.user.joinedGroups.includes(id)){state.user.joinedGroups.push(id);persist();const group=GROUPS.find(g=>g.id===id);addNotification("Group joined",`You joined ${group?.name||"a trekking group"}.`,"group");toast("You joined the group!");}else toast("You are already in this group.","info"); groupTab("browse"); }
...
function sendRequest(id){if(!state.user.sent.includes(id)){state.user.sent.push(id);persist();const person=USERS.find(u=>u.id===id);addNotification("Friend request sent",`Your request to ${person?.name||"a trekker"} was sent.`,"friend");toast("Friend request sent!");render();}}
function cancelRequest(id){state.user.sent=state.user.sent.filter(x=>x!==id);persist();toast("Request cancelled.","info");render();}
function acceptRequest(id){state.user.incoming=state.user.incoming.filter(x=>x!==id);if(!state.user.friends.includes(id))state.user.friends.push(id);persist();const person=USERS.find(u=>u.id===id);addNotification("Friend request accepted",`You are now connected with ${person?.name||"a trekker"}.`,"friend");toast("Friend request accepted!");renderProfile();}
...
function renderProfile(){
  ...
  <div class="profile-layout"><div class="profile-main"><section class="content-card"><div class="section-head compact"><h2>About</h2><button class="text-link" onclick="editProfile()">Edit</button></div><p>${esc(state.user.bio)}</p><div class="profile-details"><span><i class="fa-solid fa-location-dot"></i> ${esc(state.user.district)}, ${esc(state.user.province)}</span><span><i class="fa-solid fa-person-hiking"></i> ${esc(state.user.experience)}</span><span><i class="fa-regular fa-calendar"></i> ${esc(state.user.availability)}</span><span><i class="fa-solid fa-wallet"></i> ${esc(state.user.budget)} budget</span><span><i class="fa-solid fa-language"></i> ${state.user.languages.map(esc).join(", ")}</span></div></section>
  ...
  <section class="content-card"><div class="section-head compact"><div><h2>Saved hikes</h2><p>Your shortlist.</p></div></div><div class="mini-grid">${saved.map(trailCard).join("")||`<div class="empty full-span"><span class="empty-icon"><i class="fa-regular fa-heart"></i></span><h3>No saved trails yet</h3><p>Explore Nepal and save a few routes.</p></div>`}</div></section>
  ...
  <section class="content-card"><h3>Friends <span class="count">${friends.length}</span></h3>${friends.map(u=>`<div class="friend-row">${avatar(u,"sm")}<div><b>${esc(u.name)}</b><small>${esc(u.location)}</small></div><button class="icon-btn" onclick="openChat(${u.id})" aria-label="Message"><i class="fa-regular fa-comment"></i></button></div>`).join("")||"<p class='muted'>No friends yet.</p>"}</section>
...
function renderLogin(){
  app.innerHTML=`<section class="auth-page"><div class="auth-art"><span class="pill"> Trek Sathi</span><h1>Your next adventure starts with the right people.</h1><p>Discover Nepal's trails and connect with trekkers who match your style.</p><div class="auth-testimonial">“The easiest way to turn a solo trail idea into a group plan.”<small>— Trek Sathi community</small></div></div><div class="auth-card"><div class="auth-logo"><i class="fa-solid fa-mountain"></i></div><span class="eyebrow">WELCOME BACK</span><h2>Log in to Trek Sathi</h2><p class="muted">Use the demo account or create your own local profile.</p><form onsubmit="login(event)"><label>Email<input name="email" type="email" value="aarya@example.com" required></label><label>Password<input name="password" type="password" value="password" required></label><div class="form-row"><label class="checkbox"><input type="checkbox"> Remember me</label><a href="#" onclick="toast('Password reset would connect to your backend email service.','info');return false">Forgot password?</a></div><button class="btn primary full">Log in</button></form><div class="auth-divider">or</div><button class="btn secondary full" onclick="demoLogin()">Continue with demo account</button><p class="auth-bottom">Don't have an account? <a href="#signup">Create one</a></p></div></section>`;
}
...
document.addEventListener("click",e=>{
  if(e.target.closest("#notification-btn")) renderNotificationPanel();
